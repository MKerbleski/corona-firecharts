...
def fireplot(df, country, start_time=None, most_recent=0, save=True, show=False, fontsize=30, labelsize=30, titlesize=56, show_sum=True, split=False):# MK changed to True
    # trimming the df
    if start_time:
    ...
    if split:
        ...
        for i in range(len(dfs)):
            fireplot(dfs[i], country=country+'_'+str(i+1), split=True)
        return
...
#     ax.set_title('%s Fireplot'%country, fontsize=titlesize)
    ...
    # output
    if save:
        # plt.savefig('figures\\fire/Fire{0}_{1}.png'.format(country, date), dpi=100)
...
def fireplot(df, country, start_time=None, most_recent=0, w_ratio=1, h_ratio=1, save=True, show=False, fontsize=30, labelsize=30, titlesize=56):
    ...
    fig.tight_layout()
    if save:
        # plt.savefig('%s.png'%(country+'_'+date), dpi=40)
        # plt.savefig('figures\\fire/Fire{0}_{1}.png'.format(country, date), dpi=100)
